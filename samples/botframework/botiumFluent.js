...
driver.BuildFluent()
  .Start()
  .UserSaysText('hi bot')
  .WaitBotSaysText(5000, (text) => assert('You\'ve been invited to a survey! It will start in a few seconds...', text))
  .WaitBotSaysText(10000, (text) => assert('Hello... What\'s your name?', text))
  .UserSaysText('John')
  .WaitBotSaysText(5000, (text) => assert('Hi John, How many years have you been coding?', text))
  .UserSaysText('5')
  .WaitBotSaysText(5000, (text) => assert('What language do you code Node using?', text))
  .UserSaysText('CoffeeScript')
  .WaitBotSaysText(5000, (text) => assert('Got it... John you\'ve been programming for 5 years and use CoffeeScript.', text))
  .Stop()
  .Clean()
  .Exec()
  .then(() => {
