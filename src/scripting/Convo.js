...
class Convo {
  ...
  async runConversation (container, scriptingMemory, transcript) {
    const failSafe = (failErr, ...rest) => {
      debug(failErr)
      try {
        this.scriptingEvents.fail && this.scriptingEvents.fail(failErr, ...rest)
      } catch (err) {
      }
    }
    const failSafeAndThrow = (failErr, ...rest) => {
      failSafe(failErr, ...rest)
      throw failErr
    }
          ...
          if (convoStep.sender === 'begin' || convoStep.sender === 'end') {
            ...
            try {
              ...
              if (justAsserterError) {
                debug('Asserter error should be false in me section')
              }
            ...
            const executeOnBotHook = async (filter) => {
              let onBotResult
              try {
                onBotResult = await this.scriptingEvents.onBot({
                  convo: this,
                  convoStep,
                  container,
                  scriptingMemory,
              ...
              if (onBotResult.error) {
                if (onBotResult.justAsserterError) {
                ...
                const failErr = botiumErrorFromErr(`${this.header.name}/${convoStep.stepTag}: ${onBotResult.justAsserterError ? 'assertion' : 'critical'} error - ${onBotResult.error.message || onBotResult.error}`, onBotResult.error)
                failSafe(failErr, lastMeConvoStep)
                if (onBotResult.justAsserterError && container.caps[Capabilities.SCRIPTING_ENABLE_MULTIPLE_ASSERT_ERRORS]) {
                  assertErrors.push(onBotResult.error)
                } else {
                  throw failErr
                }
              }
            ...
            if (!botMsg || (!botMsg.messageText && !botMsg.media && !botMsg.buttons && !botMsg.cards && !botMsg.sourceData && !botMsg.nlp)) {
              ...
              failSafe(failErr, lastMeConvoStep)
              throw botiumErrorFromList([...assertErrors, failErr], {})
            }
